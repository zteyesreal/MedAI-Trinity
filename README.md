...
```mermaid
flowchart TD
    A[医生佩戴轻量化录音模块] --> B[实时采集患者口述+现场问诊对话]
    
    B --> C[MedClaw数据中枢无感拉取全域数据]
    ...
    G1 & G2 & G3 & H1 & H2 & H3 & H4 --> I[AI自动整合校验]
    
    I --> J[60秒内输出合规完整首次病程记录]
    
    J --> K[医生一键审核微调]
    
    K --> L[直接归档EMR系统<br/>全程留痕可追溯]
    
    style A fill:#e6f7ff
    style C fill:#f6ffed,stroke:#52c41a,stroke-width:2px
    style E fill:#fff7e6,stroke:#fa8c16,stroke-width:2px
    style J fill:#f0f7ff,stroke:#1890ff,stroke-width:2px
    style L fill:#f6ffed
```
...
```mermaid
graph LR
    subgraph Hospital["医院环境"]
        subgraph Dept["科室端"]
            RP[树莓派/小型机]
            MIC[录音模块]
        end
        
        subgraph Systems["医院系统"]
            HIS[HIS系统]
            LIS[LIS系统]
            PACS[PACS系统]
            EMR[EMR系统]
    ...
    style Dept fill:#e6f7ff
    style Systems fill:#fff7e6
    style LocalModel fill:#f6ffed
    style Cloud fill:#f0f7ff
```
...
graph TB
    subgraph Trinity["MedAI Trinity System"]
        ...
        LM[Medical Large Model<br/>Deep Analysis]
        MC[MedClaw Data Hub<br/>System Integration]
    end
    ...
    style Trinity fill:#f0f7ff,stroke:#1890ff,stroke-width:3px
    style SM fill:#e6f7ff
    style LM fill:#fff7e6
    style MC fill:#f6ffed
```
